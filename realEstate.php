...
<head>
  ...
  <link rel="shortcut icon" href="favicon.png">
    ...
    <style>
        ...
        .hero {
            /* background: url('images/hero-bg-real-estate.jpg') no-repeat center center; */
            padding: 60px 0;
            text-align: center;
        }
...
<body>
            ...
            <div class="row justify-content-between">
                <div class="col-lg-5">
                    <div class="intro-excerpt">
                        ...
                        <p class="mb-4">Discover the best properties available on the market. Whether you're looking for a cozy apartment or a luxurious mansion, we have something for you. Browse our listings and contact us for more details.</p>
                        <p><a href="#" target ="_blank" class="btn btn-secondary me-2">View Listings</a><a href="electrical" class="btn btn-white-outline">Explore our Electrical</a></p>
                    </div>
                </div>
                <div class="col-lg-6 text-center bg-transparent">
                    <!-- <div class="hero-img-wrap">  -->
                    <img src="images/reals.png" class="img-fluid shadow-lg shadow-md w-80 bg-transparent rounded mx-auto d-block" style="transition: transform 0.5s ease; cursor: pointer;" onmouseover="this.style.transform='scale(1.1)';" onmouseout="this.style.transform='scale(1)';" alt="Contact Image">
                 <!-- </div> -->
                </div> 
            </div>
        </div>
    ...
    <div class="container my-5">
        <h2 class="mb-5 text-center" style="font-size: 2.5rem; font-weight: 600; ">Property Listings</h2>
    
        
        <div class="filters d-flex flex-column flex-md-row align-items-center justify-content-between mb-4">
            <input type="text" id="search" class="form-control filter-input mb-3 mb-md-0" placeholder="Search for properties..." onkeyup="filterProperties()">

            <select id="type-filter" class="form-select filter-select mb-3 mb-md-0" onchange="filterProperties()">
            ...
            <select id="price-filter" class="form-select filter-select" onchange="filterProperties()">
    ...
    <div id="buyModal" class="modal">
        <div class="modal-content">
            <span class="close">&times;</span>
            <h2>Contact Us</h2>
            <form id="contactForm" class="p-4 bg-light rounded-3 shadow-sm">
            <div class="mb-3">
            <label for="name" class="form-label">Your Name</label>
            <input type="text"  id="name" name="name" class="form-control" placeholder="Your Name" required>
            </div>
            ...
            <input type="text" id="phone" name="phone"  class="form-control" placeholder="Your Phone" required>
             ...
             <div class="mb-3"> 
                <label for="address" class="form-label">Your Address</label>
               <textarea id="address" name="address"  class="form-control" placeholder="Your Message" required></textarea>
            </div>
...
<div id="productModal"  class="modal">
    ...
    <script>
        ...
        function loadProperties() {
            fetch('./components/db_conn.php?type=property')
                ...
                <div class="card h-100 shadow-sm" style="background-color: #f5f7fa; border-radius: 10px;">
                        <div class="property-item" style="min-height: 220px; overflow: hidden; border-top-left-radius: 10px; border-top-right-radius: 10px;">
                            ...
                            <p  class="card-text" style="color: #777; font-size: 0.95rem;">${item.type}</p>
                            <p class="card-text" style="color: #555; font-weight: 500;">₹${item.price}</p>
                                     <div class="d-flex justify-content-between">
                                <button class="btn btn-dark btn-sm" style="width: auto; padding: 6px 20px; font-size: 0.9rem;" onclick="contactUs(${item.id}, '${item.name}', '${item.description}', '${item.price}')">Buy Now</button>
            ...
            fetch('./components/db_conn.php?type=property')
                ...
                <div class="card h-100 shadow-sm" style="background-color: #f5f7fa; border-radius: 10px;">
                        <div class="property-item" style="min-height: 220px; overflow: hidden; border-top-left-radius: 10px; border-top-right-radius: 10px;">
                            ...
                            <p class="card-text" style="color: #555; font-weight: 500;">₹${item.price}</p>
                                    <div class="d-flex justify-content-between">
                                <button class="btn btn-dark btn-sm" style="width: auto; padding: 6px 20px; font-size: 0.9rem;" onclick="contactUs(${item.id})">Buy Now</button>
                                <button class="btn btn-light-details btn-sm" style="width: auto; padding: 6px 20px; font-size: 0.9rem;" onclick="showProductDetails(${item.id})">Details</button>
                            </div>
                        </div>
        ...
        function contactUs(productId, name, description, price) {
...
function showProductDetails(productId) {
    fetch(`./components/db_conn.php?type=property&id=${productId}`)
        .then(response => response.json())
        .then(data => {
